...
const SCORE_CONFIG: Record<string, { label: string; color: string; bg: string }> = {
  premium:  { label: "Premium ⭐",  color: "text-tf-gold-dark",  bg: "bg-tf-gold/[0.12]" },
  fiable:   { label: "Fiable ✓",   color: "text-tf-success",     bg: "bg-tf-success-bg" },
  standard: { label: "Standard",    color: "text-tf-text-muted",  bg: "bg-tf-gray-soft" },
  surveille:{ label: "Surveillé ⚠️", color: "text-tf-warning",     bg: "bg-tf-warning-bg" },
  restreint:{ label: "Restreint",    color: "text-tf-error",     bg: "bg-tf-error-bg" },
}
...
function BudgetSection({ depensePeriode }: { depensePeriode: number; range?: DateRange }) {
  ...
  const barColor    = depasse ? "var(--tf-error)" : procheLimit ? "var(--tf-warning)" : "var(--tf-success)"
  ...
  return (
    <div className="bg-white rounded-xl border border-tf-border p-5">
      ...
      {budget > 0 ? (
        <>
          {depasse && (
            <div className="flex items-center gap-2 mb-3 px-3 py-2 bg-tf-error-bg rounded-lg">
              <AlertTriangle size={13} className="text-tf-error shrink-0" />
              <p className="font-sans text-[12px] font-semibold text-tf-error">
                Budget dépassé de {formatPrix(depensePeriode - budget)}
              </p>
            </div>
          )}
          {procheLimit && !depasse && (
            <div className="flex items-center gap-2 mb-3 px-3 py-2 bg-tf-warning-bg rounded-lg">
              <AlertTriangle size={13} className="text-tf-warning shrink-0" />
              <p className="font-sans text-[12px] font-semibold text-tf-warning">
          ...
          <div className="mb-3">
            ...
            <ProgressBar value={pct} color={barColor} trackClassName="w-full h-3" />
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 text-center">
            {[
              ...
              { label: "Restant",  value: formatPrix(reste),          color: depasse ? "var(--tf-error)" : "var(--tf-success)" },
...
function AlertesSection() {
  ...
  const ALERTE_ICONS: Record<string, React.ReactNode> = {
    nouvelle_boutique: <Store size={14} className="text-tf-info" />,
    nouveau_couturier: <Scissors size={14} className="text-tf-gold" />,
    article_disponible: <Package size={14} className="text-tf-success" />,
  }

  const ALERTE_BG: Record<string, string> = {
    nouvelle_boutique:  "bg-tf-info-bg border-tf-info/30",
    nouveau_couturier:  "bg-tf-gold/[0.06] border-tf-gold/30",
    article_disponible: "bg-tf-success-bg border-tf-success/30",
  }
    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          ...
          { label: "En cours",              value: data?.stats.nb_commandes_en_cours ?? 0, icon: <Clock size={16} className="text-tf-info" /> },
          { label: formatRangeLabel(range), value: data?.stats.nb_commandes_mois ?? 0,     icon: <TrendingUp size={16} className="text-tf-success" /> },
          { label: "Likes cumulés",         value: data?.stats.nb_likes_cumules ?? 0,      icon: <Heart size={16} className="text-tf-error" /> },
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          {data?.stats.article_plus_cher && (
            <div className="bg-white rounded-xl border border-tf-border p-4 flex items-center gap-3">
              <div className="w-9 h-9 rounded-lg bg-tf-gold/10 flex items-center justify-center shrink-0">
          ...
          {data?.stats.article_moins_cher && (
            <div className="bg-white rounded-xl border border-tf-border p-4 flex items-center gap-3">
              <div className="w-9 h-9 rounded-lg bg-tf-success-bg flex items-center justify-center shrink-0">
                <TrendingDown size={16} className="text-tf-success" />
              </div>
              <div className="min-w-0">
                ...
                <p className="font-mono text-[14px] font-bold text-tf-success">{formatPrix(data.stats.article_moins_cher.prix)}</p>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-white rounded-xl border border-tf-border p-5">
          ...
          <ProgressBar
            value={(score / 5) * 100}
            color={score >= 4.5 ? "var(--tf-gold)" : score >= 3.5 ? "var(--tf-success)" : score >= 2.5 ? "var(--tf-text-muted)" : "var(--tf-error)"}
            trackClassName="w-full h-2 mb-3"
          />
