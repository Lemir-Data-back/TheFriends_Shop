...
function BudgetSection({ depensePeriode, range }: { depensePeriode: number; range: DateRange }) {
  ...
  function handleSave() {
    ...
    else         { setBudget(0);   setBudgetTypeLocal(inputType) }
    setEditing(false)
  }
        ...
        <>
          ...
          <div className="mb-3">
            <div className="flex justify-between mb-1.5">
              ...
              <span className="font-sans text-[12px] font-bold tabular-nums" style={{ color: barColor }}>{pct}%</span>
            </div>
          ...
          <div className="grid grid-cols-3 gap-3 text-center">
            {[
              ...
              <div key={label}>
                ...
                <p className="font-sans text-[14px] font-bold tabular-nums" style={{ color }}>
                  {value}
                </p>
              </div>
            ))}
  ...
  return (
    <div className="max-w-screen-lg mx-auto px-4 py-8">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 mb-8">
        <div>
          <p className="font-sans text-[13px] text-tf-text-muted">Bonjour,</p>
          <h1 className="font-serif text-h1 text-tf-black">{user?.full_name}</h1>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <span className={`text-[11px] px-3 py-1 rounded-full font-bold ${sc.bg} ${sc.color}`}>
            {sc.label}
          </span>
          ...
          {data?.stats.article_plus_cher && (
            <div className="bg-white rounded-xl border border-tf-border p-4 flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="font-serif text-[14px] font-bold text-tf-gold">{formatPrix(data.stats.article_plus_cher.prix)}</p>
              </div>
            </div>
          )}
          {data?.stats.article_moins_cher && (
            <div className="bg-white rounded-xl border border-tf-border p-4 flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="font-serif text-[14px] font-bold text-[#2D6A4F]">{formatPrix(data.stats.article_moins_cher.prix)}</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl border border-tf-border p-5">
          ...
          <div className="flex items-center gap-4 mb-4">
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 ${sc.bg}`}>
              <p className={`font-sans text-[22px] font-bold tabular-nums ${sc.color}`}>{score.toFixed(1)}</p>
            </div>
            <div>
          ...
          <div className="w-full h-2 bg-tf-gray-soft rounded-full overflow-hidden mb-3">
            <div className="h-full rounded-full transition-all duration-700"
              style={{ width: `${(score / 5) * 100}%`, backgroundColor: score >= 4.5 ? "#C9A84C" : score >= 3.5 ? "#2D6A4F" : score >= 2.5 ? "#6B6760" : "#C0392B" }} />
          </div>
      ...
      <div className="bg-white rounded-xl border border-tf-border p-5 mb-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="font-serif text-[28px] font-bold tabular-nums text-tf-black">{formatPrix(data?.stats.total_depense ?? 0)}</p>
          </div>
          <div className="text-right">
            <p className="font-sans text-[12px] text-tf-text-muted mb-1">Sur la période sélectionnée</p>
            <p className="font-serif text-[22px] font-bold tabular-nums text-tf-gold">{formatPrix(data?.stats.depense_mois ?? 0)}</p>
          </div>
        </div>
      </div>
            ...
            {data.dernieres_commandes.map((order) => (
              <Link key={order.id} href={`/commandes/${order.id}`}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-sans text-[14px] font-bold tabular-nums text-tf-black">{formatPrix(order.montant)}</span>
                </div>
              </Link>
            ))}
          </div>
