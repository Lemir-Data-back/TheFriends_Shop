    ...
    <div className="min-h-screen bg-tf-bg">
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 py-4">
        ...
        <div className="flex items-center gap-2 mb-6 text-nav text-tf-text-muted">
          <Link href="/shopping" className="flex items-center gap-1 hover:text-tf-text transition-colors">
            <ChevronLeft size={14} />
            Shopping
          </Link>
          <span>/</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
          ...
          <div className="space-y-5">
            ...
            <Link
              ...
              className="text-label uppercase tracking-widest text-tf-text-muted hover:text-tf-gold transition-colors"
            >
              Voir la boutique
            </Link>
            ...
            <div className="flex items-baseline gap-3">
              {hasPromo ? (
                <>
                  <span className="font-serif text-display text-tf-gold leading-none">
                    {formatPrix(product.prix_promo!)}
                  </span>
                  <span className="font-serif text-h3 text-tf-text-muted line-through">
                    {formatPrix(product.prix)}
                  </span>
                </>
              ) : (
                <span className="font-serif text-display text-tf-text leading-none">
                  {formatPrix(product.prix)}
                </span>
              )}
            </div>
            ...
            <div className="flex gap-3 pt-2">
              <button
                ...
                disabled={tailles.length > 0 && !selectedTaille}
                className={cn(
                  "flex-1 py-3.5 rounded-md text-btn font-bold transition-all",
                  addedToCart
                    ? "bg-tf-success text-white"
                    : "bg-tf-gold text-tf-black hover:bg-tf-gold-light",
                  tailles.length > 0 && !selectedTaille && "opacity-50 cursor-not-allowed"
                )}
              >
              ...
              <button className="w-12 h-12 flex items-center justify-center rounded-md border border-tf-border text-tf-text-muted hover:border-tf-text bg-white transition-colors">
                <Share2 size={18} />
              </button>
            </div>
