...
from app.modules.commandes.models import OrderType, OrderStatut, EscrowStatut, CreationRequestStatut
...
class OrderUpdateStatut(BaseModel):
    ...
    delai_jours: Optional[int] = Field(None, gt=0, description="Délai de confection en jours, fixé par le couturier à l'acceptation")


class OrderNegotiationAction(BaseModel):
    """Un tour dans la conversation de négociation d'une commande sur mesure."""
    action: str = Field(..., pattern="^(proposer|contre|accepter|refuser)$")
    prix: Optional[int] = Field(None, gt=0, description="Requis pour proposer/contre")
    message: Optional[str] = Field(None, max_length=280)


class NegociationEntry(BaseModel):
    auteur: str
    action: str
...
class OrderClientInfo(BaseModel):
    """Infos client affichées au couturier/vendeur qui reçoit la commande."""
    id: int
    full_name: str
...
class OrderResponse(BaseModel):
    ...
    negociations: list[NegociationEntry] = []
    delai_confection_jours: Optional[int] = None
    created_at: datetime
    updated_at: datetime
